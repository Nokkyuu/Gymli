...
class HistoryListController {
  ...
  Future<void> delete(ApiTrainingSet item) async {
    ...
    try {
      ...
      final cache = GetIt.I<WorkoutDataCache>();
      cache.deleteTrainingSetOptimistic(exerciseId: exerciseId, setId: item.id!);
      ...
      if (setIndex > 0 && currentEntries[setIndex - 1].isHeader) {
        final bool isLastSetForHeader = (setIndex + 1 >= currentEntries.length) ||
      ...
      if (removeHeader && headerIndex >= 0 && headerIndex < currentEntries.length) {
